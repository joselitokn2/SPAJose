...
textoBusqueda.addEventListener("keyup", () => {
  ...
  for (var i = 0; i < json["candidatures"].length; i++) {
      ...
      var texto = getData(columnName, i).toLowerCase();
      var textoBuscar = textoBusqueda.value.toLowerCase();
      for (let candidature of candidatures){
        let nombre = candidature.surname.toLowerCase
        if(nombre.indexOf(textoBuscar) !== -1){
...
textoBusqueda.addEventListener("keyup", () => {
  if (textoBusqueda.value.length >= 3) {
      ...
      //Comprovamos en el si coincide el texto con cada uno de los nombres
      const nombreArray = nombreArray[0].toLowerCase();
